...
async function initWebSettings() {
    ...
    toggle.addEventListener('change', async () => {
        ...
        await window.api.setWebSettings({ webEnabled: enabled, webPassword: passwordInput.value });
    ...
    document.getElementById('web-password-apply').addEventListener('click', async () => {
        ...
        await window.api.setWebSettings({ webEnabled: toggle.checked, webPassword: pw });
...
async function initDeviceInfo() {
    ...
    if (info.webUrl) {
        ...
        document.getElementById('web-url-bar').addEventListener('click', () => {
            navigator.clipboard.writeText(info.webUrl).then(() => {
...
function initActions() {
    ...
    document.getElementById('open-save-dir').addEventListener('click', () => window.api.openSaveDir());
}

async function sendFiles() {
    if (!selectedDevice && !webTargetUrl) return showToast('请先选择目标设备或输入浏览器接收地址', 'error');
    ...
    if (webTargetUrl) {
        ...
        for (const fp of files) {
            const r = await window.api.uploadFileToWeb({ baseUrl: webTargetUrl, filePath: fp });
            if (!r.success) failed++;
        }
        hideSendProgress();
    ...
    hideSendProgress();
    if (result.success) showToast(`已发送 ${files.length} 个文件到 ${selectedDevice.name}`, 'success');
...
async function sendFolderAction() {
    ...
    if (webTargetUrl) {
        ...
        const result = await window.api.uploadFolderToWeb({ baseUrl: webTargetUrl, folderPath: folder });
        hideSendProgress();
        if (result.success) showToast(`文件夹已发送至浏览器端 (${result.successCount} 个文件)`, 'success');
        else if (result.successCount > 0) showToast(`部分完成: ${result.successCount} 成功, ${result.failedCount} 失败`, 'info');
...
function renderReceiveHistory() {
    ...
    sorted.forEach((item) => {
        ...
        if (item.type === 'folder') {
            ...
            label = (item.content || '').substring(0, 60) + ((item.content || '').length > 60 ? '...' : '');
            meta = '文本';
        }
    ...
    `;
        ...
        el.querySelector('.receive-delete-btn').addEventListener('click', (e) => {
            e.stopPropagation();
            const hasFile = (item.type === 'file' || item.type === 'clipboard-image' || item.type === 'folder') && item.savePath;
            ...
            if (hasFile) {
                const displayName = item.type === 'folder' ? (item.folderName || '此文件夹') : (item.fileName || '此文件');
...
function showDeleteModal(fileName, callback) {
    ...
    document.getElementById('del-modal-overlay').onclick = () => { cleanup(); callback('cancel'); };
    btnCancel.onclick = () => { cleanup(); callback('cancel'); };
    btnRecord.onclick = () => { cleanup(); callback('record'); };
    btnFile.onclick = () => { cleanup(); callback('file'); };
}
